...
// The component was defined but not exported correctly
const ExercisesGrid = () => {
  const exercises = [
    {
      id: "speaking",
      ...
      {exercises.map((exercise) => (
        <Link href={`/exercises/${exercise.id}`} key={exercise.id} className="block">
          <Card className="h-full transition-all hover:shadow-md">
            <CardHeader className={cn("flex flex-row items-center gap-4 p-6", exercise.color)}>
              {exercise.icon}
              <div>
...
// Add the correct export statement
export { ExercisesGrid }
